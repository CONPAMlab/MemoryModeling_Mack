...
lb = [lo(1) * ones(nSS,1);  lo(2) * ones(nSS,1); -.5 * ones(8,1); zeros(12,1); lo(3)];
ub = [hi(1) * ones(nSS,1);  hi(2) * ones(nSS,1); .5 * ones(8,1); ones(12,1); hi(3)];
...
res.psB      = p((2 * nSS + 5):(2 * nSS + 8));
res.wts      = p((2 * nSS + 9):(end-1));
