function res = dpFit(R,T,D,SS,lo,hi)
...
lb = [lo(1) * ones(nSS,1);  lo(2) * ones(nSS,1); -.5 * ones(4,1); zeros(12,1); lo(3)];
ub = [hi(1) * ones(nSS,1);  hi(2) * ones(nSS,1); .5 * ones(4,1); ones(12,1); hi(3)];

x0 = genX0(nIter,R,T,D,SS,lb,ub);

maxfeval = 10000;
options = optimset('TolX',1e-8,'maxfunevals',maxfeval, ...
    'maxiter',round(maxfeval/3),'GradObj','off','Display','iter', ...
...
%     x0(iIter,:)'
    ...
    [x, ll] = fmincon(@(x) dpLL(R,T,D,SS,x),x0(iIter,:)', ...
...
res.betaM    = p((1 * nSS + 1):(2 * nSS));
res.pgB      = p((2 * nSS + 1):(2 * nSS + 4));
res.wts      = p((2 * nSS + 5):(end-1));
