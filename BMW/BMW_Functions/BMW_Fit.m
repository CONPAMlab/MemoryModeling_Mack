...
if strcmp(FitOptions.Algorithm,'fminsearch')
        %% fminsearch
...
%         FitOptions.fminconOptions.Verbosity='off';
        FitOptions.fminsearchOptions = optimset('TolX',1e-6);
        eval(['[Param_BMW, LP_BMW, Exitflag, Output]=fminsearch(@(Param)',Model,'(Param, Data, Config), Constraints.start, FitOptions.fminsearchOptions);'])

elseif strcmp(FitOptions.Algorithm,'fmincon: sqp')
        %% fmincon: sqp
        % Matlab Optimization Toolbox
        % https://ww2.mathworks.cn/help/optim/ug/fmincon.html;jsessionid=e52720ed65cadadf60d137ef2b4c#d117e83832
        if ~exist('fmincon','file')
...
%         FitOptions.fminconOptions.Verbosity='off';
        FitOptions.fminconOptions = optimoptions(@fmincon,'Algorithm','sqp','MaxIterations',1500,'Display','off');
...
elseif strcmp(FitOptions.Algorithm,'GA')
        ...
        eval(['[Param_BMW, LP_BMW, Exitflag, Output] = ga(@(Param)' Model, '(Param, Data, Config), length(Constraints.start), [], [], [], [], Constraints.lb, Constraints.ub, []);'])
