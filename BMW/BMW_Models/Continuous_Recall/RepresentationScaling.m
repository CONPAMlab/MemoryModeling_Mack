...
function Output=RepresentationScaling(param, Data, Input)

% Specify parameters
SS=Data.SS;
SS_range=unique(SS);
dprime=param(1)./SS_range; % memory strength
...
conf=zeros(length(error_range),1);
for i = 1:length(error_range)
    conf(i) = exp(-kappa*abs(error_range(i)));
end
