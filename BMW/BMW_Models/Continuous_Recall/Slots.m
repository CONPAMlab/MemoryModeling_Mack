%% Slots-plus-Averaging (Zhang & Luck, 2008)
%
...
function Output=Slots(param, Data, Input)

if isfield(Input,'Variants') && any(strcmp(Input.Variants,'Category (Within-Item)'))
    Output=Categorical_Slots_WI(param,Data,Input);
elseif isfield(Input,'Variants') && any(strcmp(Input.Variants,'Category (Between-Item)'))
    Output=Categorical_Slots_BI(param,Data,Input);
else
    % Specify parameters
    K=param(1); % Capacity
    kappa1=param(2); % Precision
    power=param(3); % Power-Law
    Nparam=3;
    ...
    SS=Data.SS;
    SS_range=unique(SS);
    % kappa for each set size
    kappa_SS=kappa1./SS_range.^power;
