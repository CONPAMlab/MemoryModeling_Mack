%% Categorical Slots-plus-Averaging (Within-Item, Mixture)
%
...
function Output=Categorical_Slots_plus_Averaging_WI_mix(param, Data, Input)
    ...
    for i_N=1:length(SS_range)
        ...
        if K<N % Beyond capacity
            if any(strcmp(Input.Variants,'ResponseNoise'))
                ...
                for i_error=1:length(error_range)
                    error0=(error_range(i_error)+bias)/(period/360);
                    p_error(i_N,i_error)=(1-K/N)*1/length(error_range)+...
            ...
            if any(strcmp(Input.Variants,'ResponseNoise'))
                for i_error=1:length(error_range)
                    ...
                    p_error_c(i_N,i_error)=b(1-p_c)*(1/length(error_range))+p_c*(esseli(0,conv_1_c)./(2*pi*besseli(0,kappa_c)*besseli(0,kappa_r)));
                end
            else
                for i_error=1:length(error_range)
                    error0=error_range(i_error)+bias;
                    ...
                    p_error_c(i_N,i_error)=(1-p_c)*(1/length(error_range))+p_c*exp(kappa_c.*cosd(error0))./(2*pi*besseli(0,kappa_c));
                end
            end
...
%         end
        for i_s=1:length(sample_range)
            ...
            nc(i_N,i_s)=sum(p_error(i_N,s_ind).*(p_error_c(i_N,c_ind)));
        end
    end
