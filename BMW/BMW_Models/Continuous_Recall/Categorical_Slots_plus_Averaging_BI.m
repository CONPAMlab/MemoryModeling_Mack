...
if ~any(strcmp(Input.Variants,'CatSelectivity'))
    kappa_cs = 0;
else
    kappa_cs = 50;
end
...
if kappa_cs == 0
    ...
    errors_c=CircDist_BMW('Diff',repmat(responses,[1,length(category_range)]),categories,period);
else
    categories = (category_range * ones(1,length(responses)))';
    errors_c=CircDist_BMW('Diff',repmat(responses,[1,length(category_range)]),categories,period);
    L_cs = exp(kappa_cs.*cosd(errors_c/(period/360)))./(2*pi*besseli(0,kappa_cs));
    p_cs = L_cs./(sum(L_cs,2));
end
    ...
    if continuous==1
        ...
        for i_N=1:length(SS_range)
            ...
            if K<N % Beyond capacity
                ...
                kappa_high=kappa_1*(floor(K/N)+1);
                kappa_low=kappa_1*floor(K/N);
                if kappa_high > 709, kappa_high = 709; end
                if kappa_low > 709, kappa_low = 709; end
                if any(strcmp(Input.Variants,'ResponseNoise'))
                    for i_error=1:length(error_range)
                        error0=error_range(i_error)+bias;
                        conv_1_c=sqrt((kappa_c).^2+(kappa_r)^2+2*kappa_c*kappa_r.*cosd(error0/(period/360)));
                ...
                if sum(p_error(i_N,:)) ~=0
                    p_error(i_N,:)=p_error(i_N,:)/sum(p_error(i_N,:));
                end
                if sum(p_error_c(i_N,:)) ~=0
        ...
        for i=1:length(errors)
            ...
            if kappa_cs ~= 0
                if categories(i)~=0
                L_c = 0;
                for k_cs = 1:length(category_range) % weighted sum of lh across categories
                    L_c = L_c + p_cs(i,k_cs)*p_error_c(SS_range==SS(i),error_range==errors_c(i,k_cs));
                end
                p_T(i)=(1-s)*((1-p_cc)*p_error(SS_range==SS(i),error_range==errors(i))+...
                    p_cc*(L_c));
                else
                    p_T(i)=(1-s)*p_error(SS_range==SS(i),error_range==errors(i));
                end
            else
                if categories(i)~=0
                    p_T(i)=(1-s)*((1-p_cc)*p_error(SS_range==SS(i),error_range==errors(i))+...
                        p_cc*p_error_c(SS_range==SS(i),error_range==errors_c(i)));
                else % slot model when cat = 0
                    p_T(i)=(1-s)*p_error(SS_range==SS(i),error_range==errors(i));
                end
            end
        end
